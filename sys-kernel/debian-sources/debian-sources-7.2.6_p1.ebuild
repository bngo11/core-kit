...
KERNEL_TRIPLET="${PV/${VERSION_SUFFIX}/}"
if [ ${PR} != "r0" ]; then
	VERSION_SUFFIX+="-${PR}"
fi
...
src_prepare() {
	...
	eapply "${FILESDIR}"/latest/extra_cpu_optimizations_7_2.patch || die
	...
	if use custom-cflags; then
		...
		if [ -n "$MARCH" ]; then
			...
			if [ "$MARCH" == "-march=znver5" ];then
				CONFIG_MARCH="CONFIG_MZEN5"
			elif [ "$MARCH" == "-march=znver3" ];then
				CONFIG_MARCH="CONFIG_MZEN3"
			fi
	...
	tweak_config .config CONFIG_CRYPTO_CRYPTD m
